...
class Automation:
        ...
        if doc_type == "교육수료증명서":
            try:
                ...
                for rows in user_rs:
                    ...
                    self.wb = load_workbook(self.makeFilePath)
        ...
        try:
            ...
            for idx, rows in enumerate(user_rs, start=1):
                for index in range(len(user_rs)):
                    if rows[index] == None:
                        member_dict[user_query_list[index]] = ""
                    else:
        ...
        try:
            ...
            for idx, rows in enumerate(rs, start=3):
                ...
                if pre_class != now_class:
                    ...
                    self.ws.cell(row=idx + class_cnt, column=1).font = Font(size = 22)
    ...
    def gatherPictures(self, exam):
        ...
        for rows in rs_user:
            ...
            try:
                shutil.copy(doc_path, dir_path + "\\{}.jpg".format(re_name))
            except:
                doc_path = self.basePath + "{}\\{}{}\\{}\\{}{}_{}.JPG".format(rows[0], rows[0], rows[1], rows[2], rows[0], rows[1], rows[2])
                try:
                    shutil.copy(doc_path, dir_path + "\\{}.jpg".format(re_name))
